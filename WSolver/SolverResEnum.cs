...
namespace W.Expressions.Solver
{
    ...
    class ResEnum
    {
        ...
        public ResEnum(ResultInfo[] results, string[][] outputFields, string[] outFieldsOrder, int[] keyFields)
        {
        ...
        IEnumerable<Timed<object[]>> EnumerateTimed(ColumnStack[] colInfos, int iSrc, TimeRange prevTimeRange)
        {
            if (iSrc == srcInfos.Length)
            #region Return result of enumeration
            {   // only return one result
                if (prevTimeRange.endTime < prevTimeRange.time)
                    yield break;
                ...
                if (!allValNulls)
                {
                    yield return new Timed<object[]>(prevTimeRange.time, prevTimeRange.endTime, res);
                    yield break;
                }
                else yield break;
            }
            ...
            if (!si.active)
            {
                ...
                foreach (var res in EnumerateTimed(colInfos, iSrc + 1, prevTimeRange))
                    yield return res;
                yield break;
            }
            #endregion
            ...
            int[] outNdxs = null;
            int nRowsFound = 0;
            var keysTimeRange = prevTimeRange;
            if (iSrc == 0)
            ...
            {   // rows filtered by key field ( 'SelectMany'-like function)
                ...
                {
                    throw new Exception(nameof(Solver) + "." + nameof(ResEnum) + ": part of key values is undefined ("
                        + string.Join(",", inOuts.Select(i => colInfos[si.loc2glb[i]]).Where(c => c.Empty).Select(c => c.name))
                        + ")");
                }
                else if (withKnown)
                    rows = si.data.BinarySearchAllWithTimeRange(inOuts, keyVals, keysTimeRange);
            ...
            var prevEndTime = prevTimeRange.time;
            if (rows != null)
            {
                foreach (var iRow in rows)
                {
                    nRowsFound++;
                    var rowDict = si.data[iRow];
                    var rowVals = rowDict.ValuesList;
                    var nextRange = prevTimeRange;
                    // adapt time range
                    if (rowDict.Time > nextRange.time)
                        nextRange.time = rowDict.Time;
                    if (rowDict.EndTime < nextRange.endTime)
                        nextRange.endTime = rowDict.EndTime;
                    if (nextRange.time > nextRange.endTime)
                        yield break;
                    if (DateTime.MinValue < prevEndTime && prevEndTime < nextRange.time)
                    {   // emulate empty values of current source
                        foreach (int i in outNdxs)
                            colInfos[si.loc2glb[i]].Push(-1, null);
                        foreach (var res in EnumerateTimed(colInfos, iSrc + 1, new TimeRange(prevTimeRange.time, nextRange.time)))
                            yield return res;
                        foreach (int i in outNdxs)
                    ...
                    foreach (int i in outNdxs)
                    {
                        ...
                        if (to != null)
                            ...
                            col.Push(iRow, null);
                        else if (nextRange.time > DateTime.MinValue)
                            col.Push(iRow, TimedObject.Timed(nextRange.time, nextRange.endTime, rv));
            ...
            if (prevEndTime < prevTimeRange.endTime)
            {   // emulate ending empty value of current source, if needed
                var lastRange = new TimeRange(prevEndTime, prevTimeRange.endTime);
                // push null values
                foreach (int i in outNdxs)
                    colInfos[si.loc2glb[i]].Push(-1, null);
                // process remained sources
