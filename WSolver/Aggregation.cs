	...
	public static class DataLoader
	{
		...
		static class FuncDefs_NoAggr
		{
			...
			public static object AGGR(CallExpr call, Generator.Ctx ctx)
			{
				...
				var expr = new CallExpr("SolverTimedObjects", new CallExpr("SolverTimesOfIDicts", new ReferenceExpr(DataLoader.sData)), ce.args[ce.args.Count - 1]);
		...
		static class FuncDefs_Aggr
		{
			...
			public static object AGGR(CallExpr call, Generator.Ctx ctx)
			{
				...
				args.Add(CallExpr.let(new ReferenceExpr(sFactory), new CallExpr("SolverWeightedSumsFactory", ce.args[0], new ReferenceExpr(sTimeA), new ReferenceExpr(sTimeB))));
				...
				var expr = new CallExpr("SolverTimedObjects", new CallExpr("SolverWeightedSumsFactoryTimes", new ReferenceExpr(sFactory)), exprAgg);
				return Generator.Generate(expr, ctx);
			}
			...
			static Expr AggImpl(CallExpr ce, Generator.Ctx ctx)
			{
				...
				if (iAkk >= 0)
					...
					resultExpr = new CallExpr("SolverWeightedSumsAdd", refAkk, new CallExpr("SolverWeightedSumsNew", new ReferenceExpr(sFactory), valsExpr, keysExpr));
			...
			static object AggImplSimple(CallExpr ce, Generator.Ctx ctx)
			{
				...
				var expr = new BinaryExpr(ExprType.Fluent, AggImpl(ce, ctx), new CallExpr("SolverWeightedSumsGet", new ConstExpr(what)));
				return Generator.Generate(expr, ctx);
			}
